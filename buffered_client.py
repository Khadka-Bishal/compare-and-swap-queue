...
class ClaimIntent(Intent):
    ...
    def apply(self, data: dict):
        ...
        # First, look for a standard queued job
        ...
        # If nothing is queued, look for a timed-out in_progress job to reclaim
        for job in data.get("jobs", []):
        ...
        for job in data.get("jobs", []):
            if job["id"] == self.job_id:
                if job["state"] == "in_progress" and job["claimed_by"] == self.worker_id:
                    ...
                    # Store idempotency token so duplicate pushes are ignored
